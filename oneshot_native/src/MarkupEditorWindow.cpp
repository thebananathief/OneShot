...
namespace oneshot
{
    ...
    static HCURSOR GetMarkupDrawingCursor()
    {
        static HCURSOR cursor = []() -> HCURSOR
        {
            ...
            candidates.emplace_back(L"C:\\Windows\\Cursors\\aero_pen.cur");
            candidates.emplace_back(L"C:\\Windows\\Cursors\\pen.cur");
            ...
            for (const auto& candidate : candidates)
            {
                ...
                if (HCURSOR loaded = static_cast<HCURSOR>(LoadImageW(nullptr, candidate.c_str(), IMAGE_CURSOR, 0, 0, LR_LOADFROMFILE | LR_DEFAULTSIZE)))
                {
                    return loaded;
                }
            }
